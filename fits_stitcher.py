...
def derive_name(path, all_paths):
    ...
    parent dirs aren't unique (a flat directory with `*.fits`)."""
    if len(all_paths) > 1:
        parents = [os.path.basename(os.path.dirname(os.path.abspath(p)))
                   for p in all_paths]
...
def fill_gaussian_weight(out, wcs, beam_center, fwhm_deg):
    ...
    Uses haversine on raw arrays; ~50x faster than SkyCoord.separation
    ...
    for y0 in range(0, ny, chunk):
        ...
        a = (np.sin(0.5 * (dec - dec0))**2
             + np.cos(dec0) * np.cos(dec) * np.sin(0.5 * (ra - ra0))**2)
...
def load_input(input_fits, name, scratch_dir):
    ...
    if os.path.exists(wpath):
        ...
        tmp = wpath + ".tmp"
        w = np.lib.format.open_memmap(tmp, mode="w+",
                                      dtype=np.float32, shape=data.shape)
...
def stitch(matched, output_fits, scratch_dir):
    ...
    if MAKE_CUTOUT and SAVE_PER_POINTING_CUTOUTS:
        ...
        for e in entries:
            ...
            fits.writeto(os.path.join(per_dir, f"{e['name']}.fits"),
                         weighted, hdr, overwrite=True)

    # Free memmap handles before final writes
    for e in entries:
        del e["weights"], e["data"]
    del entries
